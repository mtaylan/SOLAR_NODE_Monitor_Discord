...
###
#
# SOLAR Node check & Discord alert script
# Delegate mtaylan
##
...
#
. $HOME/SOLAR_NODE_Monitor/solar-mon-discord.conf
...
#DISCORD API to send notification for Latency.
...
#DISCORD API to send notification for CPU.
...
#DISCORD API to send notification for SW.
...
#DISCORD API to send notificaiton for HDD.
...
#DISCORD API to send notificaiton for BLOCKS.
...
STATUS_HEALTHY=$(curl -sS --max-time 50 --retry 20 --retry-delay 2 --retry-max-time 40 http://dapi.solar.network/api/peers/$SOLAR_NODE_IP | jq -r '.data.latency')
...
#               echo " >>>> : $STATUS_HEALTHY"
        ...
        BLOCK_STATUS_SOLAR=$(curl -sS http://dapi.solar.network/api/blockchain | jq .'[].block.height')
         BLOCK_STATUS_NODE=$(curl -sS --max-time 40 --retry 20 --retry-delay 2 --retry-max-time 40 http://dapi.solar.network/api/peers/$SOLAR_NODE_IP |  jq  '.data.height')
